    ...
    <div class="container-fluid py-4">
        <div class="card">
            ...
            <div class="card-body pt-4 p-3">
                <form action="{{ route('admin.contents.update') }}" method="POST" role="form text-left">
                    @csrf
                    @if($errors->any())
                            ...
                            <div class="form-group">
                                ...
                                <div class="@error('name') border border-danger rounded-3 @enderror">
                                    <input class="form-control" type="text" placeholder="Enter a Content Name"
                                           id="content-name" name="name" value="{{ $content->name }}">
                                    @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <div class="@error('url') border border-danger rounded-3 @enderror">
                                    <input class="form-control" type="text" placeholder="Enter the Content URL"
                                           id="content-url" name="url" value="{{ $content->url }}">
                                    @error('url')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
