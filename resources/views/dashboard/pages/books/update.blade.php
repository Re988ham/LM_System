            ...
            <div class="card-body pt-4 p-3">
                <form action="{{ route('admin.books.update') }}" method="POST" enctype="multipart/form-data" role="form text-left">
                    ...
                    <div class="row">
                                ...
                                <div class="@error('book.specialization')border border-danger rounded-3 @enderror">
                                    <select class="form-control" type="text" name="specialization_id"
                                        ...
                                        @foreach($specializations as $id => $name)
                                            <option value="{{ $id }}" @if($book->specialization_id === $id) selected @endif>{{ $name }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <input class="form-control" type="hidden" name="id" value="{{ $book->id }}">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="@error('book.url')border border-danger rounded-3 @enderror">
                                    <input class="form-control" type="text" name="url" value="{{ $book->url }}"
                                           placeholder="Enter a Book URL" id="book-url">
                                    @error('url')
                                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
